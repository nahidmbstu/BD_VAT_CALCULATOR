module.exports.inclusiveVat = function(arr) {
  if (Array.isArray(arr)) {
    if (arr.length > 0) {
      ...
      let totalSum = arr.map((a, i) => (Number(a) - Number(NewArray[i])).toFixed(2));
...
module.exports.exclusiveVat = function(arr) {
  if (Array.isArray(arr)) {
    if (arr.length > 0) {
      let NewArray = [];

      arr.map(item => {
        let vat_added = Number(item * (15 / 100)).toFixed(2);
        NewArray.push(vat_added);
      });
      let floatTotalVat = NewArray.reduce((a, b) => Number(a) + Number(b));
      let totalVat = Number.parseFloat(floatTotalVat).toFixed(2);
